...
export default defineComponent({
  ...
  setup() {
    ...
    let articleList = ref([])
    // 获取技术文章总数
    const artArticleCount = ref();
    store.dispatch('getTypeArticleCount',  {id: 1}).then(res => {
      artArticleCount.value = res
    })
    // 获取最新文章
    store.dispatch('getArticleListById', {size: artSize.value, id: 1}).then(res => {
      ...
      const {appearMore} = useAppearMore(isEmpty, articleList, artSize)
    ...
    //   }
    // }
    const onLoadMore = useLoadMore(articleList, "getArticleListById", artArticleCount, isMore, {
      size: artSize, id: 1
    })
    return {
      articleList,
