...
export default defineComponent({
  ...
  setup() {
    ...
    const heatSize = ref(5);
    // 获取文章总数
    const store = useStore();
    let articleList = ref([])
    const articleTotal = computed(() => store.state.articleTotal); 
    // 获取最新文章
    store.dispatch('getHeatArticleList', {size: heatSize.value}).then(res => {
      ...
      const {appearMore} = useAppearMore(isEmpty, articleList, heatSize)
    ...
    //   }
    // }
    const onLoadMore = useLoadMore(articleList, "getHeatArticleList", articleTotal, isMore, {size: heatSize})
    return {
      articleList,
      isEmpty,
      onLoadMore,
