...
export default defineComponent({
  ...
  setup() {
    ...
    const photoSize = ref(5);
    // 获取文章总数
    const store = useStore();
    let articleList = ref([])
    // 获取摄影文章总数
    const photoArticleCount = ref();
    store.dispatch('getTypeArticleCount',  {id: 2}).then(res => {
      photoArticleCount.value = res
    })
    // 获取最新文章
    store.dispatch('getArticleListById', {size: photoSize.value, id: 2}).then(res => {
      ...
      const {appearMore} = useAppearMore(isEmpty, articleList, photoSize)
    ...
    //   }
    // }
    const onLoadMore = useLoadMore(articleList, "getArticleListById", photoArticleCount, isMore, {
      size: photoSize, id: 2
    })
    return {
      articleList,
